...
def get_list_models(url):
    response = requests.get(url, verify=False, headers={"Accept":"application/json"})
    if response.status_code != 200:
        print(f"Error: Unable to fetch the list from {url}. HTTP Status code: {response.status_code}")
        return None
...
list_url = f"{base_url}{method}"
models = get_list_models(list_url)
print(models)
# Example usage:
overall_graph = Graph()
for model_name in models:
    ...
    if rdf_graph:
        ...
        overall_graph +=rdf_graph
        #json = parse_json_from_url(url)
        #print("Title from JSON: " + json['title'])


overall_turtle_data = overall_graph.serialize(format='turtle')
filename = "output-overall.ttl"
with open(filename, "wb") as f:
    f.write(overall_turtle_data.encode())
